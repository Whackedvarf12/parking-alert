import {parkirnaMesta} from "../front-end/assets/js/dummyData/parkingdata";
class KParking {
    #naslov;
    #mesto;
    #latituda;
      ...
      this.#casovniZig = null;
        ...
        this.redarInter = setInterval(() => {
            ...
            redarji.forEach((redar) => {
                let oddaljenost = izracunajRazdaljoVMetrih(this.#latituda, this.#longituda, redar.latitude, redar.longitude);
                let predvidevanaOddaljenost = izracunajRazdaljoVMetrih(this.#latituda, this.#longituda, redar.expected.latitude, redar.expected.longitude);
                //Glede na trenutno lokacijo
                if(oddaljenost <= 250){
                    //TODO sproži alert.
                    let opozorilo = new Opozorilo((Math.floor(Math.random() * 10000000) + 1), redar, oddaljenost, new Date(), this);
                    opotorilo.posljiOpozorilo();
                    //TODO neki, da se preveri, ali je opozorilo potrjeno
                }
                //Glede na predvideno lokacijo
                if(oddaljenost <= 500){
                    //TODO sproži alert.
                    let opozorilo = new Opozorilo((Math.floor(Math.random() * 10000000) + 1), redar, predvidevanaOddaljenost, new Date(), this);
                    opotorilo.posljiOpozorilo();
                    //TODO neki, da se preveri, ali je opozorilo potrjeno
                }
            })
    ...
    najdiNajblizjiParking(){
        ...
        parkirnaMesta.forEach((elem)=>{
            if(selPark == null){
                ...
                if(izracunajRazdaljoVMetrih(this.#latituda, this.#longituda, selPark.latitude, selPark.longitude) > izracunajRazdaljoVMetrih(this.#latituda, this.#longituda, elem.latitude, elem.longitude)){
                    selPark = elem;
                }
            }
        });
